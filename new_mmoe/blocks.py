...
# 添加这个装饰器来注册模型
@register_keras_serializable()
class DINLayer(layers.Layer):
    """
    DIN注意力池化层
    输入: query (候选item), keys (历史序列)
    输出: 注意力加权后的历史特征聚合
    """
    
    def __init__(self, hidden_units=[80, 40], activation='sigmoid', **kwargs):
    ...
    def build(self, input_shape):
        ...
        for i, units in enumerate(self.hidden_units):
            self.attention_layers.append(
                layers.Dense(units, activation='relu', name=f'att_fc_{i}')
            )
        
        # 注意力输出层
        self.attention_output = layers.Dense(1, activation=self.activation, name='att_out')
    ...
    def call(self, inputs, training=None):
        """
        inputs: [query, keys]
        - query: [batch_size, embedding_dim] 候选item embedding
        - keys: [batch_size, seq_len, embedding_dim] 历史序列embedding
        """
        query, keys = inputs
        
        batch_size = tf.shape(query)[0]
        seq_len = tf.shape(keys)[1]
        embedding_dim = tf.shape(query)[1]
        
        # 扩展query维度以匹配keys
        query_expanded = tf.tile(
            tf.expand_dims(query, 1), 
            [1, seq_len, 1]
        )  # [batch_size, seq_len, embedding_dim]
        
        # 构造注意力特征: [query, keys, query-keys, query*keys]
        diff = query_expanded - keys
        prod = query_expanded * keys
        
        # 拼接所有特征
        attention_input = tf.concat([
            query_expanded, keys, diff, prod
        ], axis=-1)  # [batch_size, seq_len, 4*embedding_dim]
        
        # 通过注意力网络
        attention_score = attention_input
        for layer in self.attention_layers:
            attention_score = layer(attention_score)
        
        # 计算注意力权重
        attention_weights = self.attention_output(attention_score)  # [batch_size, seq_len, 1]
        attention_weights = tf.squeeze(attention_weights, axis=-1)  # [batch_size, seq_len]
        
        # 直接进行Softmax归一化，不使用mask
        attention_weights = tf.nn.softmax(attention_weights, axis=1)
        attention_weights = tf.expand_dims(attention_weights, axis=-1)  # [batch_size, seq_len, 1]
        
        # 加权聚合历史特征
        output = tf.reduce_sum(keys * attention_weights, axis=1)  # [batch_size, embedding_dim]
        
        return output
    
    def get_config(self):
        config = super(DINLayer, self).get_config()
        config.update({
